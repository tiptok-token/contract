...
contract TipTok is Context, IBEP20, Ownable {
    ...
    uint256 public limitToSellBeforeBlockCooldown = 5000000000000 * 10**9; // 0.5% of total Supply
    ...
    address public marketingAddr = 0x0A539Ab493a74aEe1855752902560D18C1E82325;
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(
            taxFee <= 10,
            "Percentage too high. Please use a lower percentage."
        );
        _taxFee = taxFee;
    }
    ...
    function setMaxAllowedBalance(uint256 maxBalancePercent)
        external
        onlyOwner()
    {
        require(
            maxBalancePercent >= 1,
            "Percentage too low. Please use a higher percentage."
        );
        _maxAllowedBalanceAmount = _tTotal.mul(maxBalancePercent).div(10**2);
    }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(
            ...
            "Amount is too low. Please set a higher maxTxAmount."
        );
    ...
    function setSellLimitForBlockCooldown(uint256 SellLimitForBlockCooldown)
        external
        onlyOwner()
    {
        require(
            SellLimitForBlockCooldown >= 5000000000000 * 10**9,
            "Amount is too low. Please set a higher SellLimitForBlockCooldown."
        );
    ...
    function setBlocksToWait(uint256 newBlocks) external onlyOwner() {
        require(
            newBlocks <= 1440,
            "Block wait time is too hight. Set it below than 1440."
        );
        ...
        )
    {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) =
            _getRValues(tAmount, tFee, tMarketing, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rFee,
        ...
        )
    {
        ...
        uint256 tTransferAmount =
            tAmount.sub(tFee).sub(marketing).sub(tLiquidity);
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            // Owner can send to the zero address on demand.
        ...
        if (to == uniswapV2Pair && !_isAdminAccount[from]) {
            require(
                block.number >= amountBlockToSellNext[from],
                "You have sold or transferred too much, try to sell again in 1400 blocks from your last transaction."
            );
            require(
                amount <= _maxTxAmount,
        ...
        if (to != uniswapV2Pair && !_isAdminAccount[from]) {
            require(
                (currentBalance + amount) <= _maxAllowedBalanceAmount,
                "Account balance exceeds the maxAllowedBalanceAmount."
            );
            require(
                block.number >= amountBlockToSellNext[from],
                "You have sold or transferred too much, try to sell again in 1400 blocks from your last transaction."
